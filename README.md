...
├── Taskfile.yml        # Task definitions (primary script management)
├── deploy.sh           # Legacy deploy script (use: task deploy)
├── serve.sh            # Legacy dev server (use: task serve)
...
We use [Taskfile](https://taskfile.dev/) as the primary way to manage repo-wide scripts. All common operations should be added as tasks in `Taskfile.yml`.
...
To add new repo-wide scripts, update `Taskfile.yml` instead of creating standalone shell scripts. This provides:
